...
import { OtpLogs, User } from '../../models';
import { v4 as uuid4 } from 'uuid';
import { addMinutes, getIstTime } from '../../utils/dateTime';

export const updatePasswordDb = async (filters: { [key: string]: any }, password: string): Promise<dbRes> => {
    const defaultResp: dbRes = {
        result: null,
    };
    try {
        const resp: any = await User.updateOne(filters, { password });
...
export const saveOtpDb = async ({ otp, resourceId, resourceType = OTP_RESOURCE_TYPES.EMAIL, requestSource }: IsaveOtp): Promise<dbRes> => {
    ...
    try {
        ...
        const expiryOn = addMinutes(createdOn, parseInt(process.env.OTP_EXPIRY_MIN));
